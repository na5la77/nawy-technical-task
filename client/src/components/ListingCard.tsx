        ...
        <Card className="w-full h-full overflow-hidden flex flex-col justify-between items-stretch transition-shadow duration-800 ease-in-out hover:shadow-lg dark:hover:shadow-primary/25">
          ...
          <CardContent className="p-4 flex flex-col flex-grow">
            <div className="flex justify-between items-start mb-2">
              <div>
                <h3 className="text-base sm:text-lg font-semibold text-foreground capitalize">{listing.unit_name}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground">{listing.unit_number}</p>
                <p className="text-xs sm:text-sm text-muted-foreground">{listing.project}</p>
              </div>
              <Badge variant="secondary" className="text-xs sm:text-sm">{listing.unit_type}</Badge>
            </div>
            <div className="flex items-center space-x-4 mb-2">
              <div className="flex items-center">
                <Bed className="w-4 h-4 mr-1 text-muted-foreground" />
                <span className="text-sm text-muted-foreground">{listing.bedrooms}</span>
              </div>
              <div className="flex items-center">
                <Bath className="w-4 h-4 mr-1 text-muted-foreground" />
                <span className="text-sm text-muted-foreground">{listing.bathrooms}</span>
              </div>
              <div className="flex items-center">
                <Proportions className="w-4 h-4 mr-1 text-muted-foreground" />
                <span className="text-sm text-muted-foreground">  {listing.size ?? 25}m&sup2;</span>
              </div>
            </div>
          ...
          <CardFooter className="bg-muted p-4">
            <p className="text-lg sm:text-xl font-bold text-foreground">${listing.price.toLocaleString()}</p>
          </CardFooter>
        </Card>
      </Link>
  )
