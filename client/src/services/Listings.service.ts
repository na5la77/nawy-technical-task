...
const API_URL =   process.env.NEXT_PUBLIC_DOCKER === "TRUE"
    ? process.env.NEXT_PUBLIC_SERVER_URL_DOCKER
    : process.env.NEXT_PUBLIC_SERVER_URL;

export async function getListings(page: number = 1, limit: number = 8, filters: ListingsFilter = {}): Promise<ListingsResponse> {
...
export async function getListing(id: string): Promise<ListingDetailsResponse> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/listings/${id}`);
