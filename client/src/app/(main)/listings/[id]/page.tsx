...
export default async function ListingPage({ params }: { params: { id: string } }) {
    ...
    return (
        <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <h1 className="text-2xl sm:text-3xl font-bold mb-6 text-foreground capitalize">{listing.unit_name}</h1>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                        <p className="text-xl sm:text-2xl font-bold mb-2 sm:mb-0 text-foreground">${listing.price.toLocaleString()}</p>
                        <Badge variant="secondary" className="text-base sm:text-lg">{listing.unit_type}</Badge>
                    </div>
                    ...
                    <div className="flex items-center space-x-4 mb-4">
                        <div className="flex items-center">
                            ...
                            <span className="text-foreground">{listing.bedrooms} Bedrooms</span>
                        </div>
                        <div className="flex items-center">
                            <Bath className="w-5 h-5 mr-2 text-muted-foreground"/>
                            <span className="text-foreground">{listing.bathrooms} Bathrooms</span>
                        </div>
                        <div className="flex items-center">
                            <Proportions className="w-5 h-5 mr-2 text-muted-foreground"/>
                            <span className="text-foreground">  {listing.size ?? 25}m&sup2;</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-muted p-4 rounded-lg mb-6">
                        ...
                        <p><strong>Name:</strong> {listing.user.name}</p>
                        <p><strong>Email:</strong> {listing.user.email}</p>
                        <p><strong>Phone:</strong> {listing.user.phoneNumber}</p>
                    </div>
                </div>
            </div>
        </div>
